...
int main(int argc, char **argv) {
    ...
    // goes. On Windows and Linux this is also the taskbar icon.
    {
        ...
        // The PNGs first, when they are there: they carry the artwork drawn
        // for small sizes, which a single SVG scaled down cannot match.
        ...
        // And the master SVG when they are not. The PNGs are generated by
        // assets/icon/build-icons.sh and deliberately not committed, so a
        // clone has none of them - which on Linux meant the window and the
        // task bar showed the toolkit's default, and the About box a blank
        // square. The SVG *is* committed, and Qt6::Svg is already a
        // dependency, so there is no reason to have no icon at all.
        if (icon.isNull()) {
            for (const QString &folder : {QStringLiteral("../Resources/appicon"),
                                          QStringLiteral("appicon")}) {
                const QString svg =
                    here.absoluteFilePath(folder + QStringLiteral("/jt-filework.svg"));
